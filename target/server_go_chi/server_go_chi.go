...
func (server) FileExtension() string {
	return "go"
}

func (server) Generate(data target.Proto) ([]byte, error) {
	var indent = "\t"
	...
	headersWriter.WriteString(indent + "\"github.com/go-chi/chi/v5\"\n")
	headersWriter.WriteString(")\n\n")
	...
	for _, srv := range data.Services {
		...
		for _, rpc := range srv.RPCs {
			...
			serverWriter.WriteString(indent + "mux.Post(\"/" + rpc.Name + "\", func(w http.ResponseWriter, r *http.Request) {\n")
